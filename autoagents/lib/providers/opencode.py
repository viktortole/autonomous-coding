...
Runs prompts through the opencode CLI and adapts output to
the same message loop shape used by the Claude SDK client.
"""
...
class OpenCodeClient:
    ...
    Provides the same interface as ClaudeSDKClient for compatibility.
    """
    ...
    def __init__(self, project_dir: Path, model: str | None = None) -> None:
        ...
        Args:
            project_dir: Working directory for commands
    ...
    async def receive_response(self) -> AsyncIterator[AssistantMessage]:
        """
        Execute the query and yield the response.

        Yields:
            AssistantMessage containing the CLI output
        """
