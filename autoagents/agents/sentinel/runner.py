    ...
    try:
        ...
        while True:
            ...
            for result in quick_results:
                ...
                print(f"     {status_emoji} {result.config.component}: {status_color}{result.message}{Style.RESET} ({result.duration_ms}ms)")
            ...
            if errors and state.repairs_today < RATE_LIMITS["max_repairs_per_hour"]:
                for error in errors:
                    if "HTTP" in error.config.component or "Server" in error.config.component:
                        print(f"\n  {SENTINEL_EMOJI['repairing']} Attempting dev server repair...")
                        repair_result = repair_workflow.execute_workflow("dev_server_restart")
                        state.repairs_today += 1
                        state.restarts_today += 1 if repair_result.status.value == "success" else 0
                        if repair_result.status.value == "success":
